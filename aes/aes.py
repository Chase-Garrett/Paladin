#!/usr/bin/env python3.12

# AES encryption and decryption schemes as outlined by the NIST specifications

BLOCK_SIZE = 16

# Rijndael S-box
s_box = []

# Inverted Rijndael S-box
inv_s_box = []

# Look up table for rcon
rcon_lookup = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]

# used for polynomial multiplication in a finite field
xtime = lambda a: (((a << 1) ^ 0x1b) & 0xff) if (a & 0x80) else (a << 1)


# Round Constant
def rcon(i: int) -> bytes:
    rcon_value = bytes([rcon_lookup[i-1], 0, 0, 0])
    return rcon_value


# Substitute word
def substitute_word(word: [int]) -> bytes:
    substituted_word = bytes(s_box[i] for i in word)
    return substituted_word


# XOR bytes
def xor(x: bytes, y: bytes) -> bytes:
    return bytes([x ^ y for (x, y) in zip(x, y)])


# Rotate word
def rotate_word(word: [int]) -> [int]:
    return word[1:] + word[:1]


# Expand key
def key_expansion(key: bytes, word: int = 4) -> [[[int]]]:
    nk = len(key) // 4

    key_bit_length = len(key) * 8

    if key_bit_length == 128:
        num_rounds = 10
    elif key_bit_length == 192:
        num_rounds = 12
    else:
        num_rounds = 14

    w = state_from_bytes(key)

    for i in range(nk, word * (num_rounds + 1)):
        temp = w[i-1]
        if i % nk == 0:
            temp = xor(substitute_word(rotate_word(temp)), rcon(i // nk))
        elif nk > 6 and i % nk == 4:
            temp = substitute_word(temp)
        w.append(xor(w[i - nk], temp))

    return [w[i * 4:(i + 1) * 4] for i in range(len(w) // 4)]

# add round key
def add_round_key(state: [[int]], key_schedule: [[[int]]], round: int):
    round_key = key_schedule[round]
    for r in range(len(state)):
        state[r] = [state[r][c] ^ round_key[r][c] for c in range(len(state[0]))]


# The following two functions implement sub_bytes for aes encryption and decryption
def sub_bytes(state: [[int]]):
    for r in range(len(state)):
        state[r] = [s_box[state[r][c]] for c in range(len(state[0]))]


def inverse_sub_bytes(state: [[int]]):
    for r in range(len(state)):
        state[r] = [inv_s_box[state[r][c]] for c in range(len(state[0]))]


# The following two functions implement shift_rows for aes encryption and decryption
def shift_rows(state: [[int]]):
    state[0][1], state[1][1], state[2][1], state[3][1], = state[1][1], state[2][1], state[3][1], state[0][1]
    state[0][2], state[1][2], state[2][2], state[3][2], = state[2][2], state[3][2], state[0][2], state[1][2]
    state[0][3], state[1][3], state[2][3], state[3][3], = state[3][3], state[0][3], state[1][3], state[2][3]


def inverse_shift_rows(state: [[int]]):
    state[1][1], state[2][1], state[3][1], state[0][1], = state[0][1], state[1][1], state[2][1], state[3][1]
    state[2][2], state[3][2], state[0][2], state[1][2], = state[0][2], state[1][2], state[2][2], state[3][2]
    state[3][3], state[0][3], state[1][3], state[2][3], = state[0][3], state[1][3], state[2][3], state[3][3]


# The following four functions are used to implement the mixing of columns in aes encryption and decryption
# one function for each mixes individual columns and the one function for each loop each column through the
# function that mixes the individual columns
def mix_column(col: [int]):
    col_0_temp = col[0]

    xor_all = col[0] ^ col[1] ^ col[2] ^ col[3]

    col[0] ^= xor_all ^ xtime(col[0] ^ col[1])
    col[1] ^= xor_all ^ xtime(col[1] ^ col[2])
    col[2] ^= xor_all ^ xtime(col[2] ^ col[3])
    col[3] ^= xor_all ^ xtime(col_0_temp ^ col[3])


def inverse_mix_column(col: [int]):
    x = xtime(xtime(col[0] ^ col[2]))
    y = xtime(xtime(col[1] ^ col[3]))
    col[0] ^= x
    col[1] ^= y
    col[2] ^= x
    col[3] ^= y


def mix_columns(state: [[int]]):
    for row in state:
        mix_column(row)


def inverse_mix_columns(state: [[int]]):
    for row in state:
        inverse_mix_column(row)
    mix_columns(state)


# Define two-dimensional array
def state_from_bytes(data: bytes) -> [[int]]:
    state = [data[i * 4:(i + 1) * 4] for i in range(len(data) // 4)]
    return state


# Retrieve plain/cipher text from array
def bytes_from_state(state: [[int]]) -> bytes:
    text = bytes(state[0] + state[1] + state[2] + state[3])
    return text


# main aes algorithm for encryption
def aes_encryption(data: bytes, key: bytes) -> bytes:
    state = state_from_bytes(data)

    key_schedule = key_expansion(key)

    add_round_key(state, key_schedule, round=0)

    key_bit_length = len(key) * 8

    if key_bit_length == 128:
        num_rounds = 10
    elif key_bit_length == 192:
        num_rounds = 12
    else:
        num_rounds = 14

    for round in range(1, num_rounds):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, key_schedule, round)

    sub_bytes(state)
    shift_rows(state)
    add_round_key(state, key_schedule, round=num_rounds)

    cipher_text = bytes_from_state(state)

    return cipher_text


# main aes algorithm for decryption
def aes_decryption(cipher_text: bytes, key: bytes) -> bytes:
    key_byte_length = len(key)
    key_bit_length = key_byte_length * 8

    if key_bit_length == 128:
        num_rounds = 10
    elif key_bit_length == 192:
        num_rounds = 12
    else:
        num_rounds = 14

    state = state_from_bytes(cipher_text)
    key_schedule = key_expansion(key)
    add_round_key(state, key_schedule, round=num_rounds)

    for round in range(num_rounds-1, 0, -1):
        inverse_shift_rows(state)
        inverse_sub_bytes(state)
        add_round_key(state, key_schedule, round)
        inverse_mix_columns(state)

    inverse_shift_rows(state)
    inverse_sub_bytes(state)
    add_round_key(state, key_schedule, round=0)
    plain_text = bytes_from_state(state)

    return plain_text


# adds padding to messages before encryption
def pad(data: bytes) -> bytes:
    padding_length = BLOCK_SIZE - (len(data) % BLOCK_SIZE)
    padded_data = data + bytes([padding_length] * padding_length)
    return padded_data


# Encryption based on CBC mode of operation for AES
def cbc_encrypt(plain_text: bytes, key: bytes, iv: bytes) -> bytes:
    padded_text = pad(plain_text)
    cipher_text = []
    p_1 = padded_text[:BLOCK_SIZE]
    c_1 = aes_encryption(xor(p_1, iv), key)
    cipher_text += c_1
    c_i_1 = c_1
    for i in range(1, len(padded_text) // BLOCK_SIZE):
        p_i = padded_text[i * BLOCK_SIZE:(i+1) * BLOCK_SIZE]
        c_i = aes_encryption(xor(p_i, c_i_1), key)
        cipher_text += c_i
        c_i_1 = c_i

    return bytes(cipher_text)


# Decryption based on CBC mode of operation for AES
def cbc_decrypt(cipher_text: bytes, key: bytes, iv: bytes) -> bytes:
    plain_text = []
    c_1 = cipher_text[:BLOCK_SIZE]
    o_1 = aes_decryption(c_1, key)
    p_1 = xor(o_1, iv)
    plain_text += p_1

    c_i_1 = c_1
    for i in range(1, len(cipher_text) // BLOCK_SIZE):
        c_i = cipher_text[i * BLOCK_SIZE:(i+1) * BLOCK_SIZE]
        o_i = aes_decryption(c_i, key)
        p_i = xor(o_i, c_i_1)
        plain_text += p_i
        c_i_1 = c_i

    return bytes(plain_text)
